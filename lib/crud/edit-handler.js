async function editHandler ({ req, reply, coll, view = 'bajoAdmin:/coll/form', redirect = 'bajoAdmin:/coll/:coll/list', tpl = {} } = {}) {
  const { pascalCase, defaultsDeep, error } = this.bajo.helper
  const { recordGet, recordUpdate } = this.bajoWeb.helper
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (err.message === 'redirect') await reply.redirectTo(redirect, { params: req.params })
    }
  }
