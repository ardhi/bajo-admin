...
async function listHandler ({ ctx, req, reply, coll, view = 'bajoAdmin:/coll/stat', tpl = {} } = {}) {
  ...
  if (!stats.includes(stat)) throw error('Invalid stat type. Must be one of these: %s', stats.join(', '))
  if (!aggs.includes(agg)) throw error('Invalid aggregate type. Must be one of these: %s', aggs.join(', '))
  if (!charts.includes(chart)) throw error('Invalid chart type. Must be one of these: %s', charts.join(', '))
